...
// 魔力值（Mana）HUD渲染器，用于显示玩家的魔力值和魔力条
public class ManaBar {
    ...
    private void renderManaValue(TextRenderer font, DrawContext context, int x, int y, PlayerEntity player) {
        ...
        // 渲染魔力值文本（格式：当前值/最大值）
        text = BarHelper.KeepOneDecimal(Mana) + "/" + BarHelper.KeepOneDecimal(MaxMana);
        context.drawText(font,text,x+10,y-9,0x4D6090,false); // 使用蓝色调表示魔力
    }
    ...
    private void renderManaBar(DrawContext context, float tickDelta, int x, int y, PlayerEntity player) {
        ...
        // 动态计算健康条总宽度（基于最大生命值）
        int totalBarWidth = 30 + (int) ((maxMana - 50) * 3); // 20点基础血量对应80像素
        totalBarWidth = MathHelper.clamp(totalBarWidth, 60, 300); // 限制在80-200像素之间

        // 计算当前生命值比例
        float healthProportion = mana/maxMana;
